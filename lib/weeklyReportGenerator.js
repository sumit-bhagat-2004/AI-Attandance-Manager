import { subjects, calculateTotalClassesHeld, getEffectiveCycleStartDate } from './scheduleData';
...
export function generateWeeklyReport(userData, weekStartDate, studentName = null, hints = null, generateStudyPlan = true) {
  try {
    const weekRange = getWeekDateRange(weekStartDate);
    const weekInCycle = getWeekInCycle(weekStartDate, getEffectiveCycleStartDate(userData));
...
export function generateAllWeeklyReports(userData, studentName = null, hints = null) {
  try {
    const reports = [];
    const cycleStart = getEffectiveCycleStartDate(userData);
    const now = new Date();
